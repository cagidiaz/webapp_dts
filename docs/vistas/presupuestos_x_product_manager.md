...
| **Vendedor** | Selector con búsqueda (`SearchableSelect`) | Filtra qué comercial ha generado las ventas de los productos del PM seleccionado. Dispone de botón "Limpiar" en cabecera. | Permite al PM analizar qué comerciales están vendiendo mejor sus productos o en qué zonas se requiere apoyo técnico. |
| **Familia** | Selector con búsqueda (`SearchableSelect`) | Filtra por familia de producto de Business Central con botón dedicado de "Limpiar". | Segmentar por tipología técnica o gamas de producto. |
| **Subfamilias** | Selector múltiple con búsqueda (`MultiSearchableSelect`) | Permite filtrar por una o múltiples subfamilias simultáneamente. Incluye buscador rápido, acciones de marcar/desmarcar todas, contador visual y botón de "Limpiar (N)". | Comparar o agregar múltiples subcategorías de producto en una sola consulta. |
| **Código Producto (SKU)** | Input con botón de borrado rápido | Permite buscar o filtrar por un código de artículo específico o referencia de fabricante. Incluye botón directo `X` dentro del campo y botón "Limpiar" en su etiqueta. | Analizar un equipo o consumible concreto en todos los clientes que lo han adquirido. |
| **Buscador de Cliente** | Input con debounce (400ms) | Búsqueda rápida por nombre o código de cliente sin perder el foco de escritura (`keepPreviousData`). | Localizar un cliente concreto dentro del catálogo del PM. |
| **Limpiar Filtros** | Botones contextuales y globales | Botón global `X` en cabecera + botones "Limpiar" individuales en cada filtro activo + píldoras de alto contraste en los selectores. | Retornar rápidamente a la visión global o retirar filtros selectivamente con máxima visibilidad. |
